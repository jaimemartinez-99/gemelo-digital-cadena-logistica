...
object AlmacenMaster {
  case class IniciarAlmacenMaster(localizaciones: Seq[Localizacion], dtI: DateTime, dt0: DateTime)
...
class AlmacenMaster extends Actor with ActorLogging {
  ...
  override def receive: Receive = {
    case IniciarAlmacenMaster(localizaciones, dtI, dt0) =>
      ...
      for (i <- referencias.indices) {
        ...
        almacenes(i).ref ! ResetearAlmacen(almacenes(i).id, almacenes(i).localizacion, dtI, dt0)
      }
